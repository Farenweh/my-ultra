...
def test_msda_fastpath_cache_is_instance_local_and_autograd_safe():
    module = MSDeformAttn(d_model=32, n_levels=2, n_heads=4, n_points=4)
    copied_module = deepcopy(module)
    assert copied_module._msda_fastpath_cache is not module._msda_fastpath_cache

    with torch.inference_mode():
        copied_tensors = module_utils._get_msda_fastpath_tensors(
            [(4, 4), (2, 2)], torch.device("cpu"), copied_module._msda_fastpath_cache
        )

    assert not module._msda_fastpath_cache
    assert len(copied_module._msda_fastpath_cache) == 1
    assert all(not torch.is_inference(tensor) for tensor in copied_tensors)

    value = torch.ones(1, requires_grad=True)

    class SaveForBackward(torch.autograd.Function):
        @staticmethod
        def forward(ctx, tensor, metadata):
    ...
    SaveForBackward.apply(value, copied_tensors[0]).sum().backward()
    assert value.grad is not None

    cached_tensors = module_utils._get_msda_fastpath_tensors(
        [(4, 4), (2, 2)], torch.device("cpu"), copied_module._msda_fastpath_cache
    )
    assert all(cached is original for cached, original in zip(cached_tensors, copied_tensors))
    replacement = module_utils._get_msda_fastpath_tensors(
        [(8, 8), (4, 4)], torch.device("cpu"), copied_module._msda_fastpath_cache
    )
    assert all(new is not old for new, old in zip(replacement, copied_tensors))
    assert len(copied_module._msda_fastpath_cache) == 1

    copied_module.to(dtype=torch.float16)
    assert not copied_module._msda_fastpath_cache
