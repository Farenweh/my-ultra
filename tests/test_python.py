...
def test_coco_json_eval_remaps_string_image_ids_in_memory(tmp_path):
    """验证官方评估会稳定映射字符串 image id，且不改写源 JSON。"""
    from ultralytics.models.yolo.detect.val import DetectionValidator

    source_image_id = "ffe05a10b9f9a6a54a6ce34fead72c87"
    ...
    annotations["images"][0]["id"] = source_image_id
    annotations["annotations"][0]["image_id"] = source_image_id
    annotation_file.write_text(json.dumps(annotations), encoding="utf-8")
    prediction_file = tmp_path / "predictions.json"
    predictions = [{"image_id": source_image_id, "category_id": 1, "bbox": [10, 20, 30, 40], "score": 0.9}]
    ...
    annotation_input, prediction_input, image_id_map, mapping_file = validator._prepare_coco_eval_inputs(
        prediction_file, annotation_file
    )

    assert annotation_input["images"][0]["id"] == 1
    assert annotation_input["annotations"][0]["image_id"] == 1
    assert prediction_input[0]["image_id"] == 1
    assert image_id_map[validator._coco_image_id_key(source_image_id)] == 1
    assert json.loads(annotation_file.read_text(encoding="utf-8"))["images"][0]["id"] == source_image_id
    assert json.loads(prediction_file.read_text(encoding="utf-8"))[0]["image_id"] == source_image_id
    assert json.loads(mapping_file.read_text(encoding="utf-8"))["images"] == [
        {"evaluation_image_id": 1, "source_image_id": source_image_id}
    ]
    assert (image_dir / "0001.jpg").is_file()


def test_rtdetr_coco_json_dataset_dispatches_train_and_val(tmp_path):
    """验证 RT-DETR 训练和验证都会读取 COCO JSON，同时保持方形缩放。"""
    from ultralytics.models.rtdetr.train import RTDETRTrainer
    from ultralytics.models.rtdetr.val import RTDETRCOCODataset, RTDETRValidator
    ...
    cfg.task = "detect"
    cfg.imgsz = 64
    builder = SimpleNamespace(args=cfg, data=data)

    train_dataset = RTDETRTrainer.build_dataset(builder, data["train"], mode="train", batch=1)
    val_dataset = RTDETRValidator.build_dataset(builder, data["val"], mode="val", batch=1)

    assert isinstance(train_dataset, RTDETRCOCODataset)
    assert isinstance(val_dataset, RTDETRCOCODataset)
    assert train_dataset.split == "train"
    assert val_dataset.split == "val"
    assert train_dataset.labels[0]["image_id"] == 7
    assert val_dataset.labels[0]["image_id"] == 7
    assert train_dataset.load_image(0)[2] == (64, 64)
    assert val_dataset[0]["img"].shape == (3, 64, 64)
    assert val_dataset[0]["image_id"] == 7


def test_rtdetr_yolo_dataset_dispatch_remains_unchanged(tmp_path):
    """验证 RT-DETR 的传统 YOLO TXT 数据集仍走原有加载路径。"""
    from ultralytics.models.rtdetr.train import RTDETRTrainer
    from ultralytics.models.rtdetr.val import RTDETRDataset

    image_dir, _ = _write_coco_json_dataset(tmp_path, [{"id": 1, "name": "ship"}])
    label_dir = tmp_path / "labels"
    label_dir.mkdir()
    (label_dir / "0001.txt").write_text("0 0.25 0.5 0.3 0.5\n", encoding="utf-8")
    data_yaml = tmp_path / "yolo.yaml"
    data_yaml.write_text(
    ...
    dataset = RTDETRTrainer.build_dataset(builder, data["train"], mode="train", batch=1)

    assert type(dataset) is RTDETRDataset
    assert dataset.labels[0]["cls"].tolist() == [[0.0]]
    assert dataset.load_image(0)[2] == (64, 64)
