...
RUN_REAL_MODEL = os.getenv("RUN_C_RADIO_V4_SO400M_TESTS") == "1"


@pytest.mark.slow
@pytest.mark.skipif(not NPU_AVAILABLE or not RUN_REAL_MODEL, reason="需要显式启用C-RADIOv4-SO400M真实NPU测试")
def test_c_radio_v4_so400m_official_checkpoint_forward_and_backward(monkeypatch):
    """验证官方SO400M权重、动态分辨率、AMP和首层融合attention梯度。"""
    torch.npu.set_device("npu:0")
    torch.npu.set_compile_mode(jit_compile=False)
    model = CRADIOv4("so400m", pretrained=True).eval().requires_grad_(False).to("npu:0")
    ...
    # 完整SO400M模型反向成本较高；首个block覆盖融合attention输出、输入梯度和QKV权重梯度。
    block = model.model.blocks[0].train().requires_grad_(True)
    source = torch.randn(1, 1610, 1152, device="npu:0", dtype=torch.float16)
