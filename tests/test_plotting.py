...
def test_plot_results_writes_overview_and_split_plots(tmp_path, monkeypatch, loss_names, expected_loss_files):
    ...
    axvline_calls, scatter_calls, text_calls, legend_calls = [], [], [], []
    axvline, scatter, text, legend = Axes.axvline, Axes.scatter, Axes.text, Axes.legend
    ...
    def text_spy(self, x, y, text_value, *args, **kwargs):
        text_calls.append(text_value)
        return text(self, x, y, text_value, *args, **kwargs)
    ...
    assert any(text.startswith("best train ") for text in text_calls)
    assert any(text.startswith("last val ") for text in text_calls)
    assert legend_calls
...
def test_plot_results_avoids_checkpoint_label_overlaps(tmp_path, monkeypatch):
    """验证完全重合的 checkpoint 标签仍能分离、保持可见并连接到原始数据点。"""
    layouts = {}

    def savefig_spy(self, fname, *args, **kwargs):
        fname.parent.mkdir(parents=True, exist_ok=True)
        fname.write_bytes(b"plot")
        if fname.parent.name != "results":
            return
        self.canvas.draw()
        ax = self.axes[0]
        renderer = self.canvas.get_renderer()
        labels = [text for text in ax.texts if text.get_text().startswith(("best ", "last "))]
        layouts[fname.name] = {
            "labels": labels,
            "label_boxes": [text.get_bbox_patch().get_window_extent(renderer) for text in labels],
            "axes_box": ax.get_window_extent(renderer),
            "legend_box": ax.get_legend().get_window_extent(renderer),
            "arrow_count": len(ax.patches),
        }
    ...
    results_csv.write_text(
        "\n".join(
            (
                "epoch,time,train/box_loss,metrics/mAP50-95(B),val/box_loss,fitness,lr/pg0",
                "1,10,2.0,0.3,2.0,0.1,0.001",
                "2,20,1.5,0.2,1.5,0.2,0.002",
                "3,30,1.0,0.1,1.0,0.9,0.003",
            )
        )
        + "\n",
        encoding="utf-8",
    ...
    assert {"loss_box.png", "loss_total.png", "metric_map50_95_box.png"} == layouts.keys()
    for filename, layout in layouts.items():
        labels = layout["labels"]
        label_boxes = layout["label_boxes"]
        assert label_boxes
        assert layout["arrow_count"]
        for i, box in enumerate(label_boxes):
            intersection = Bbox.intersection(box, layout["axes_box"])
            assert intersection is not None
            assert intersection.bounds == pytest.approx(box.bounds)
            assert not box.overlaps(layout["legend_box"])
            overlaps = [
                (labels[i].get_text(), labels[j].get_text(), box.bounds, other.bounds)
                for j, other in enumerate(label_boxes[i + 1 :], start=i + 1)
                if box.overlaps(other)
            ]
            assert not overlaps, (filename, overlaps)
