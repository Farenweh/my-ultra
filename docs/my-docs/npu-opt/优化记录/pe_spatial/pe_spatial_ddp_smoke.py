"""使用torchrun验证两卡冻结PE-Spatial-L骨干网络的检测训练。"""
...
def main() -> None:
    ...
    try:
        cfg = Path(__file__).resolve().parents[3] / "ultralytics/cfg/models/rf-det/pe-spatial-yolo11.yaml"
        ...
        dynamic_cache_names = {"_frequency_cache", "_sin_cache", "_cos_cache", "_pos_embed_cache"}
        ...
        image = torch.rand(1, 3, 644, 644, device=device)
        ...
        if dist.get_rank() == 0:
            print(f"两卡PE-Spatial DDP smoke通过，平均loss={float(reduced):.6f}")
    finally:
