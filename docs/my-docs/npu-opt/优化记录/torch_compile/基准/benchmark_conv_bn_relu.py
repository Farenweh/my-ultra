...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--blocks", type=int, default=1, help="串联的 Conv-BN-ReLU 块数")
...
def main(args: argparse.Namespace) -> dict:
    """运行正确性和/或配对性能测量。"""
    # TorchNPU 在导入阶段初始化任务队列，必须先固定这一关键性能变量。
    ...
    class ConvBNReLUStack(nn.Module):
        ...
        def __init__(self, in_channels: int, out_channels: int, blocks: int):
            ...
            for index in range(blocks):
                layers.extend(
                    (
                        nn.Conv2d(in_channels if index == 0 else out_channels, out_channels, 3, padding=1, bias=False),
    ...
    if shape[1] <= 0 or args.out_channels <= 0 or args.blocks <= 0:
        raise ValueError("输入通道数、输出通道数和块数必须为正整数")
    ...
    master = ConvBNReLUStack(shape[1], args.out_channels, args.blocks)
    initial_state = clone_state(master)
    parameter_count = sum(parameter.numel() for parameter in master.parameters())
    ...
    def make_model():
        model = ConvBNReLUStack(shape[1], args.out_channels, args.blocks).to(device)
