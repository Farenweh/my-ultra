...
if CRADIO_V3_ATTENTION_BACKEND not in _VALID_ATTENTION_BACKENDS:
    raise ValueError(
        f"不支持CRADIO_V3_ATTENTION_BACKEND={CRADIO_V3_ATTENTION_BACKEND!r}；"
        f"可选值为{sorted(_VALID_ATTENTION_BACKENDS)}"
    )
...
def _supports_npu_fusion_attention(q: Tensor, k: Tensor, v: Tensor, num_heads: int) -> bool:
    if not IS_ASCEND or q.device.type != "npu" or q.ndim != 4:
        return False
    if q.shape != k.shape or q.shape != v.shape or q.dtype not in {torch.float16, torch.bfloat16}:
        return False
    if k.dtype != q.dtype or v.dtype != q.dtype or k.device != q.device or v.device != q.device:
        return False
    batch, sequence, heads, head_dim = q.shape
    if not batch or not sequence or heads != num_heads or head_dim not in {64, 80} or batch * heads >= 1000:
        return False
    if _ascend_jit_compile_enabled() or not _npu_attention_device_supported(q.device.index or 0):
        return False
    try:
        return hasattr(_get_torch_npu(), "npu_fusion_attention")
    except (AttributeError, ImportError, RuntimeError):
        return False


def select_attention_backend(q: Tensor, k: Tensor, v: Tensor, num_heads: int) -> Literal["fusion", "sdpa"]:
    if CRADIO_V3_ATTENTION_BACKEND == "sdpa":
        return "sdpa"
    supported = _supports_npu_fusion_attention(q, k, v, num_heads)
    if CRADIO_V3_ATTENTION_BACKEND == "fusion" and not supported:
        raise RuntimeError(
            "CRADIO_V3_ATTENTION_BACKEND=fusion不支持当前设备、JIT模式、dtype或shape："
...
class Attention(nn.Module):
    """保持官方checkpoint键名并为Ascend保留BSND快路径的自注意力。"""

    def __init__(self, dim: int, num_heads: int, *, device=None, dtype=None):
        super().__init__()
        if dim % num_heads:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if select_attention_backend(q, k, v, self.num_heads) == "fusion":
            x = _get_torch_npu().npu_fusion_attention(
                q,
                k,
                v,
                self.num_heads,
