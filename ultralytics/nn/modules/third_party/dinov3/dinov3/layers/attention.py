...
USE_ASCEND_ROPE = IS_ASCEND and USE_DINOV3_ASCEND_ROPE
_torch_npu = None
...
class SelfAttention(nn.Module):
    ...
    def apply_rope(self, q: Tensor, k: Tensor, rope: Tensor | Tuple[Tensor, Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        assert prefix >= 0
        use_ascend_rope = USE_ASCEND_ROPE and q.device.type == "npu"
        q_prefix = q[:, :, :prefix, :]
        if use_ascend_rope:
            q = RoPEOnAscend.apply(q[:, :, prefix:, :], sin, cos)  # [B, head, hw, D//head]
        else:
            q = rope_apply(q[:, :, prefix:, :], sin, cos)  # [B, head, hw, D//head]
        q = torch.cat((q_prefix, q), dim=-2)  # [B, head, N, D//head]
        k_prefix = k[:, :, :prefix, :]
        if use_ascend_rope:
            k = RoPEOnAscend.apply(k[:, :, prefix:, :], sin, cos)  # [B, head, hw, D//head]
        else:
