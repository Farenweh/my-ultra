...
def _get_msda_fastpath_tensors(
    value_spatial_shapes: torch.Tensor | list | tuple,
    device: torch.device,
    cache: dict[tuple[str, int, tuple[tuple[int, int], ...]], tuple[torch.Tensor, torch.Tensor]] | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Get MMCV fast-path metadata from an optional per-MSDeformAttn single-entry cache."""
    ...
    if cache is not None:
        cache.clear()
        cache[cache_key] = (spatial_shapes, level_start_index)
    return spatial_shapes, level_start_index
...
def multi_scale_deformable_attn_pytorch(
    ...
    if msda_function is not None:
        spatial_shapes, level_start_index = _get_msda_fastpath_tensors(
            value_spatial_shapes, value.device, _fastpath_cache
        )
