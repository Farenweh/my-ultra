...
3. 根据需要的优化类型进行相应编译参数设置并进行编译，LTO和PGO优化可以单独使用，也可以叠加一起使用，当前torch\_npu已支持基于毕昇编译器的编译优化选项。
    ...
    - LTO+PGO优化
        ...
        - Profile数据格式转换。
            ...
            执行该步骤前，请确保系统已安装LLVM工具链，且`llvm-profdata`命令已加入PATH环境变量。执行如下命令：

            ```shell
            llvm-profdata merge /tmp/profile/*.profraw -o default.profdata
            ```

            该命令可以合并/tmp/profile目录下所有的profraw文件，profile数据文件不受机器环境影响，可以迁移到其他机器上使用。
