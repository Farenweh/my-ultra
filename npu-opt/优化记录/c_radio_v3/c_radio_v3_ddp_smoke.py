"""使用torchrun验证两卡冻结C-RADIOv3-L骨干网络的检测训练。"""
...
def main() -> None:
    ...
    try:
        cfg = Path(__file__).resolve().parents[3] / "ultralytics/cfg/models/rf-det/c-radio-v3-yolo11.yaml"
        ...
        if dist.get_rank() == 0:
            print(f"两卡C-RADIOv3 DDP smoke通过，平均loss={float(reduced):.6f}")
    finally:
